...
class MercadoLivreSpider(scrapy.Spider):
    ...
    def extract_detailed_product_info(self, response):
        ...
        try:
            ...
            event_data_pattern = r'melidata\("add",\s*"event_data",\s*(\{[^}]+\})'
            ...
            if event_match:
                try:
                    ...
                    # Corrigir fechamento de chaves se necessário
                    brace_count = json_str.count('{') - json_str.count('}')
                    if brace_count > 0:
                        json_str += '}' * brace_count
                    event_data = json.loads(json_str)
                    logging.info(f"Event data encontrados: seller={event_data.get('seller_name', 'N/A')}")
